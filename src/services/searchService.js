...
function buildDrugResult(drug, userLocation = null) {
  ...
  return {
    drug,
    inventoryResults,
    closestPharmacyResult: inventoryResults[0] ?? null,
...
export function searchMockData(rawQuery, userLocation = null) {
  const query = normalize(rawQuery ?? "")
...
export function getMockSearchRecommendations(rawQuery, userLocation = null) {
  const query = normalize(rawQuery ?? "")
  ...
  return [...suggestedDrugs, ...fallbackDrugs]
    ...
    .slice(0, 5)
    .map((item) => {
      const inventoryResults = buildInventoryResults(item.drug.id, userLocation)
      ...
      return {
        ...
        availabilityLabel: userLocation
          ? `${inventoryResults.length} near you`
          : `${inventoryResults.length} partner pharmacies`,
